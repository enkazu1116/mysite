...
export default function UserBooksManagerPage() {
  ...
  const isResolvedOwner = userId != null && userId === booksUserId.data;
  const isBooksOwner = isDeveloperMock || isResolvedOwner;
  ...
  if (!booksUserId.isLoading && (!displayUser || !userId)) {
    return (
    ...
    <LibrarySurface className="px-6 pb-8 pt-4 sm:px-10">
      <section className="mx-auto mb-5 flex max-w-4xl items-center gap-2 text-left">
        ...
        <h1 className="font-display m-0 text-xl font-semibold text-[var(--lib-ink)]">
          {displayUser?.name ?? "Books"} / Books 管理
        </h1>
      </section>
